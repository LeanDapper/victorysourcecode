...
class PlayState extends MusicBeatState
{
	...
	var blackvoid:FlxSprite = new FlxSprite(0, 0).makeGraphic(1280, 720, FlxColor.BLACK);
	var songAppear:FlxSprite;
	// API stuff
	...
	override public function create()
	{
		...
		songAppear = new FlxSprite();
		songAppear.frames = Paths.getSparrowAtlas('CoverAppear','shared');
		songAppear.animation.addByPrefix('hi','idle',24);
		songAppear.screenCenter();
	...
	function newSongIntro():Void
	{
		blackvoid.cameras = [camHUD];
		blackvoid.scrollFactor.set();
		songAppear.scrollFactor.set();
		songAppear.cameras = [camHUD];
		add(blackvoid);
		add(songAppear);
		songAppear.animation.play('hi');
		FlxG.sound.play(Paths.sound('CoverAppear','shared'), 0.7);

		new FlxTimer().start(1.7, function(funny:FlxTimer)
			{
				remove(songAppear);
				startCountdown();
				FlxTween.tween(blackvoid, {alpha: 0}, 0.3, {onComplete: function(yo:FlxTween)
				{
					remove(blackvoid);
				}});
			});
	...
	override function stepHit()
	{
		...
		if (lastStep != curStep && curSong == 'Victory' && songMultiplier == 1)
		{
			switch(curStep)
			{
				case 1984:
					if (Stage.curStage.toLowerCase() == 'victory')
					{
		...
		if (lastStep != curStep && curSong == 'endless' && songMultiplier == 1)
			{
